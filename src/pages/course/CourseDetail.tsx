...
import { useLessonChatActions, useLessonThreads, useMyLessonMessages, useThreadMessages } from "../../hooks/useLessonChat";
import { useAssignmentActions, useLessonAssignments, useSubmissionActions } from "../../hooks/useAssignments";
...
import { COURSE_FALLBACK_IMAGE, DEFAULT_ASSIGNMENT_FORM, getYouTubeEmbedUrl, type AssignmentFormState } from "./course-detail/utils";
...
function CourseDetail() {
  ...
  const [activeTab, setActiveTab] = useState<"assignments" | "chat" | "submissions">("assignments");
  ...
  const { data: assignments = [], refetch: refetchAssignments } = useLessonAssignments(activeLesson?.id ?? "");
  const { createAssignment, updateAssignment, deleteAssignment } = useAssignmentActions(activeLesson?.id ?? "");
  ...
  const { mySubmission, submissions, submitAssignment, gradeSubmission } = useSubmissionActions(selectedAssignmentId, {
  ...
  const lessonThreads = useLessonThreads(activeLesson?.id ?? "", canModerateChat);
  const threadMessages = useThreadMessages(selectedThreadId, canModerateChat);
  const { sendMyMessage, sendThreadMessage, isSending } = useLessonChatActions(activeLesson?.id ?? "", selectedThreadId);
  ...
  const switchLesson = (lesson: LessonApi) => {
    if (activeLesson && activeLesson.id !== lesson.id) void persistLessonProgress(activeLesson);
  ...
  if (courseLoading) return <div className="min-h-screen grid place-items-center text-slate-500">Loading...</div>;
  if (!course) return <div className="min-h-screen grid place-items-center text-slate-500">Course not found</div>;
  ...
  const completedPercent =
    ...
    (sortedLessons.length > 0 && activeIndex >= 0 ? Math.round(((activeIndex + 1) / sortedLessons.length) * 100) : 0);
  const youtubeEmbedUrl = getYouTubeEmbedUrl(activeLesson?.video_url);
  const videoSource = activeLesson?.video_url && !youtubeEmbedUrl ? toMediaUrl(activeLesson.video_url) : "";
  const chatMessages = canModerateChat ? (threadMessages.data ?? []) : (myLessonMessages.data ?? []);
  const isChatLoading = canModerateChat ? threadMessages.isLoading : myLessonMessages.isLoading;
  const selectedThread = (lessonThreads.data ?? []).find((item) => item.id === selectedThreadId) ?? null;
...
<section className="rounded-3xl bg-gradient-to-b from-white to-slate-50/50 p-5 shadow-xl border border-slate-200/50">
  ...
  <div className="mt-6 relative">
    <div className="h-1.5 w-full bg-gradient-to-r from-slate-200 via-slate-300 to-slate-200 rounded-full overflow-hidden">
      <div className={`h-full bg-gradient-to-r rounded-full transition-all duration-500 ease-out ${
        activeTab === "assignments" 
